...
def view_did_load():
    ...
    with st.form("BMI_Calculator",clear_on_submit=True):
        height = st.slider(":blue[請選/擇身高]",min_value = 120, max_value=230,value =DEFULT_HIGHT,key="height")  
        weight = st.number_input(":blue[請輸入體重]",step=0.5,value=DEFULT_WEIGHT,max_value= 500.0,min_value=15.0,key="weight")
    
        submitted = st.form_submit_button(label="確認並計算", use_container_width=True)
    ...
    <br>
    ''',unsafe_allow_html=True)
            did_clear = st.button("清除結果",use_container_width=True)
            if did_clear:
                submitted = False
